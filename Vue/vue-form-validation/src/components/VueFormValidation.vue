<template>
        <div class="container" style="max-width: 500px; text-align: left; float:left;">
            <div class="alert alert-success" role="alert">
                <h1> Form Validation using Vuelidator library </h1>
            </div>
        <form @submit.prevent="handleSubmit">
            <div class="form-group">
                <label for="name">Name</label>
                <input type="text" v-model="userForm.name" id="name" name="name" class="form-control" :class="{'is-invalid':isSubmitted && $v.userForm.name.$error}"/>
                 <div v-if="isSubmitted && !$v.userForm.name.required" calss="invalid-feedback">Name field is required</div>
            </div>
            <div class="form-group">
